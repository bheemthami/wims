		...
		<div class="box-body">
			<fieldset class="fieldset-border">
				...
				<div class="col-md-12">
					...
					@if($event->attachment)
					<a href ="{{ asset('uploads/events/'.$event->attachment)}}" target="_blank"><i class="fa fa-eye"></i> view</a>
					@else
			...
			<fieldset class="fieldset-border">
				...
				<div class="col-md-12">
					<div class="col-md-6">
						<p><strong>Start Date: </strong> {{ date('Y-m-d',strtotime($event->start_date))}}, {{ $event->start_time }}</p>	
					</div>
					<div class="col-md-6">
						<p><strong>End Date: </strong> {{ date('Y-m-d',strtotime($event->end_date))}}, {{ $event->end_time }}</p>	
					</div>
			...
			<fieldset class="fieldset-border">
				...
				<div class="col-md-12">
					<div class="col-md-6">
						<p><strong>Publish status: </strong> <label for="" class="label label-{{($event->status == 1) ? 'success' : 'warning'}}"> {{ ($event->status == 1) ? 'Published' : 'Draft' }} </label> </p>	
					</div>
				</div>
			</fieldset>
		</div>
